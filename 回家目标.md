...
以File结尾表示文件对象，可能是文件对象，也可能是文件夹对象：
...
对子类进行初始化时，无论使用子类还是父类的无参构造函数，都一定会调用父类的构造的函数，要不是隐式调用super()，要不是自己手动调有参的super(xxxxx)

---

因为游戏业务里面大部分数据都是关于玩家本人的，所以通过一致性hash也好（我的做法），统一分配服务器（funplus的做法）也好，保证一个玩家的一种数据同一时刻只有一个线程在处理，后续处理变成队列，把绝对一致性降级为顺序一致性，实现水平扩展了。
如果遇到同时需要修改多个所有权的业务，游戏场景大概只有好友，公会加入和退出，充值，匹配，这种场景单独拿出来写tcc或者降级为rpc，rpc失败按保底逻辑回滚。比如匹配和组队业务用每次检查和出错回滚来做，公会和好友用tcc。充值用对账逻辑来做。
其实大部分游戏业务都是只处理个人的，如果是aoe技能和战斗，你不需要强一致性，那么就用消息转发机制来做了。

横向扩展这个需要用一个中间状态来维护，系统需要保存一个扩展前状态和扩展后状态，等到目前在队列里面的数据已经处理完毕后，才切换入扩展后状态。
事件驱动是没问题的，但是状态同步也不能少，最终方案是事件驱动加状态同步共同实现的。在google dataflow思想里面，按照窗口的概念弄，在一段窗口时间后，达成一个时间点之前的一致性。在窗口中，状态是不一致的，但是在很短时间后，我们系统一般是十秒或者0.2秒各种形式的窗口之后，数据就变成针对某个时间点一致了。分布式系统中，需要事件驱动，但是事件是不可靠的，系统的故障会导致某些事件的乱序或者丢失。这种情况下，需要一个同步机制，让状态在一个时间段后一致。
...
并行是并发的一个子情况，只有的多核CPU的情况下，并发才有可能转变为并行。
...
相同点就是，cpp和java的引用在声明之后的行为都是一模一样的，比如：引用这个变量是在栈中的，不同的是java将对像完全放在堆中，cpp的对象可能一部分在栈中一部分在堆中，由程序员自己决定。
...
string d=b;//声明一个变量，并调用string的=函数，=函数是一个左值引用入参的函数，应该是进行了深拷贝
d="456";
cout<<b<<endl;//输出111
```
