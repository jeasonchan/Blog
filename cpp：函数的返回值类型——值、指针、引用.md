...
入参是   右值引用  时，能调用 void hahh(std::string&& string1)；根据函数的语义，函数的入参是一个右值引用，当实际入参是 右值 或者 右值引用 时，会先调用 转移构造 函数（如果程序员没有定义，就只能直接调用 左值引用  的 拷贝构造  函数）给函数内产生的对像续命；如果同一函数是在懒得用右值引用重载，就先用毅哥左值引用接收一下右值，这时候会调用 转移赋值 函数（同理，如果程序员没有定义，就只能直接调用 左值引用  的 拷贝赋值  函数）；

**小结两种处理右值的套路：**

1. 函数重载出一个右值引用入参的+入参的类重载出一个转移构造函数
2. 函数不仍然只需只要一个左值引用入参的+入参的类重载出一个转移赋值函数+额外用声明一个引用通过赋值符号接收右值

所以，**函数的入参最好不要定义为传值的入参类型，而是分别定义为左值引用或者右值引用**。


# 5 总结

1. 函数的入参最好不要定义为传值的入参类型，而是分别定义为左值引用或者右值引用
2. 返回值能传有效的引用就传递引用，实在想传递函数内创建的对像，可以直接返回指针  或者  直接返回一个对像（值传递）+转移构造函数/转移赋函数
