...
**在C++中，临时对象都是const类型的**。因此，如果有一个表达式试图将一个const类型的对象转换为非const类型，这是非法的。引用型参数应该在能被定义为const的情况下，尽量定义为const 。
...
//调用了string的赋值函数("=")进行赋值操作
//"123"是作为const char* 类型的入参
string str="123";
```

在这种情况下，右值本身是不能通过赋值符号去修改的，因为是一个没有姓名的const的对像/引用。但是实际上右值是可以被修改的，如 :
...
1. 为了延长右值的生命周期，且减少拷贝构造和拷贝赋值的开销，需要的重载构造函数和赋值符号，将右值引用作为入参，也就是**需要手动实现针对右值引用的转移赋值和转移构造**。

2. 如果不手动实现转移赋值和转移构造，会直接调用编译器自动实现的拷贝赋值和拷贝构造，默认的拷贝赋值和拷贝构造是浅拷贝，值成员对象会memcpy，指针成员对象就只会拷贝指针指向同一片内存。
